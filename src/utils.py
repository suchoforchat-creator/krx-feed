...
def series_change(series: Sequence[float | None]) -> tuple[Optional[float], Optional[float]]:
    values = _clean_series(series)
    if len(values) < 2:
        return None, None
    change_abs = float(values[-1] - values[-2])
    change_pct = None if values[-2] == 0 else float(change_abs / values[-2])
...
def yahoo_ohlcv(
    ...
    *,
    ...
    subset = data[list(dict.fromkeys(fields))].dropna(how="all")
    if subset.empty:
        raise ValueError(f"requested fields empty for {ticker}")
    ...
    field_data: dict[str, list[float | None]] = {}
    for field in fields:
        series = subset.get(field)
        if series is None:
            values: list[float | None] = [None] * len(subset)
        else:
            values = [None if value != value else float(value) for value in series.tolist()]
...
def yahoo_close_series(
    ...
    *,
    ...
    payload = yahoo_ohlcv(ticker, period=period, interval=interval, limit=limit, fields=("Close",))
    series = payload["fields"].get("Close", [])
    return {
